...
import { ArrowUpCircle, ArrowDownCircle, SlidersHorizontal, History, ChevronRight } from "lucide-react";
...
import { getStockHistory, type StockMovement } from "@/app/admin/ingredients/actions";
...
export function StockHistoryDrawer({
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      const data = await getStockHistory(ingredient.ingredient_id);
      setMovements(data);
    });
  }, [open, ingredient]);
                ...
                {movements.map((mov) => {
                  ...
                  return (
                    <div key={mov.movement_id} className="flex gap-3">
                      {/* icono en la línea */}
                      <div className={cn("w-7 h-7 rounded-full flex items-center justify-center shrink-0 z-10 border border-border", cfg.bg)}>
                      ...
                      <div className="flex-1 min-w-0 bg-muted/20 rounded-xl border border-border/60 px-3 py-2.5 flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-1.5 font-mono text-sm">
                          <span className="text-muted-foreground">{Number(mov.stock_before).toFixed(2)}</span>
                          <ChevronRight className="w-3 h-3 text-muted-foreground/50 shrink-0" />
                          <span className="font-semibold text-foreground">{Number(mov.stock_after).toFixed(2)}</span>
                        ...
                        <div className="flex items-center justify-between gap-2 mt-0.5">
                          {mov.notes ? (
                            <p className="text-[11px] text-muted-foreground/80 truncate">{mov.notes}</p>
                          ) : (
                            <span />
                          )}
                          {name && (
